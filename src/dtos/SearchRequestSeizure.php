...
function parseSearchRequestSeizure(array $data): array
{
    ...
    if (!isset($data['drugType']) || trim($data['drugType']) === '') {
        $drugType = null;
    } elseif (!Validator::isCommaSeparatedStrings($data['drugType'])) {
        return ['isSuccess' => false, 'message' => 'invalid drugType format'];
    } else {
    ...
    if (!isset($data['seizureType']) || trim($data['seizureType']) === '') {
        $seizureType = null;
    } elseif (!Validator::isCommaSeparatedStrings($data['seizureType'])) {
        return ['isSuccess' => false, 'message' => 'invalid seizureType format'];
    } else {
